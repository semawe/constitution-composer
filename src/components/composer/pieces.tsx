...
> = {
  core: {
    dot: "bg-slate-400",
    bar: "border-rule",
    tag: "bg-surface-muted text-body ring-rule",
    tint: "",
    chip: "hover:border-field-accent hover:text-body",
    label: "text-muted",
  },
  retirable: {
    dot: "bg-teal-500",
    bar: "border-teal-400",
    tag: "bg-accent-soft text-accent-text ring-teal-200",
    tint: "bg-accent-faint",
    chip: "hover:border-teal-400 hover:text-accent-text",
    label: "text-accent-text",
  },
  extension: {
    dot: "bg-violet-500",
    bar: "border-violet-400",
    tag: "bg-extension-soft text-extension-text ring-extension-border",
    tint: "bg-extension-faint-surface",
    chip: "hover:border-violet-400 hover:text-extension-text",
    label: "text-extension-text",
  },
  pedagogique: {
    dot: "bg-amber-400",
    bar: "border-amber-300",
    tag: "bg-warning-soft text-warning-text ring-warning-border",
    tint: "bg-warning-soft/40",
    chip: "hover:border-amber-400 hover:text-warning-text",
    label: "text-warning-text",
  },
  app: {
    dot: "bg-rose-500",
    bar: "border-rose-400",
    tag: "bg-app-soft text-app-text ring-app-border",
    tint: "bg-app-faint",
    chip: "hover:border-rose-400 hover:text-danger-text",
    ...
    <li>
      <button
        ...
        className={`group flex w-full items-start gap-2 rounded-md border px-2.5 py-2 text-left text-sm transition ${
          on
            ? `${ui.bar} ${ui.tint} text-body`
            : "border-transparent text-body hover:bg-surface-muted"
      ...
      >
        ...
        <span className="min-w-0 leading-snug">
          {mod.label}
          {premium && !on && (
            <span className="ml-1.5 inline-block align-middle rounded-full bg-surface-muted px-1.5 py-px text-[0.62rem] font-medium text-muted">
              compte
            </span>
          )}
          {locked && (
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className={`flex h-7 w-7 items-center justify-center rounded-full border bg-background transition duration-200 ${
            open
              ? "rotate-45 border-field-accent text-body"
              : "border-field text-muted opacity-40 hover:border-field-accent hover:text-body hover:opacity-100 group-hover:opacity-100"
          }`}
        >
              ...
              {modules.map((m) => {
                ...
                return (
                  <button
                    ...
                    data-add={m.id}
                    title={m.description}
                    className={`inline-flex items-center gap-1.5 rounded-full border border-dashed border-field px-3 py-1 text-xs text-muted transition ${ui.chip}`}
                  >
  ...
  if (!editing) {
    return (
      <button
        ...
        className="mt-4 inline-flex items-center gap-1.5 rounded-full border border-dashed border-field px-3 py-1 text-xs text-muted transition hover:border-field-accent hover:text-body"
      >
        <span className="text-base leading-none">+</span>
  ...
  return (
    <div className="mt-4 rounded-md border border-rule bg-surface/70 p-4">
      <div className="mb-2 flex items-center justify-between">
        ...
        <span
          className={`text-xs ${
            paraCount > 4 ? "text-warning-muted" : "text-muted"
          }`}
        >
      ...
      <textarea
        ...
        className="doc-prose w-full resize-y rounded border border-field bg-surface p-3 text-[0.98rem] leading-relaxed text-body outline-none transition focus:border-field-accent"
      />
      {paraCount > 4 && (
...
export function Legend({
  ...
  return (
    <details className="group/leg mt-8 border-t border-rule pt-4">
      <summary className="flex cursor-pointer list-none items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-muted">
        {ui.legend}
        <svg
          viewBox="0 0 16 16"
      ...
      <ul className="mt-2 space-y-1.5">
        {rows.map((r) => (
          <li key={r.key} className="flex items-center gap-2 text-xs text-muted">
